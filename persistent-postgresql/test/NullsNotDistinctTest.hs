...
specs = describe "NULLS NOT DISTINCT support" $ do
    let runDb = runConnAssert

    it "generates correct SQL for NULLS NOT DISTINCT constraint" $ do
        let alterWithNND = AddUniqueConstraint 
                (ConstraintNameDB "unique_nnd_email")
                [FieldNameDB "name", FieldNameDB "email"]
                ["!nullsNotDistinct"]
        
        let alterWithoutNND = AddUniqueConstraint 
                (ConstraintNameDB "unique_standard_email")
                [FieldNameDB "name", FieldNameDB "email"]
                ["!force"]
        
        let tableName = EntityNameDB "test_table"
        let sqlWithNND = showAlterTable tableName alterWithNND
        let sqlWithoutNND = showAlterTable tableName alterWithoutNND
        
        sqlWithNND `Hspec.shouldBe` 
            "ALTER TABLE \"test_table\" ADD CONSTRAINT \"unique_nnd_email\" UNIQUE NULLS NOT DISTINCT(\"name\",\"email\")"
        
        sqlWithoutNND `Hspec.shouldBe`
            "ALTER TABLE \"test_table\" ADD CONSTRAINT \"unique_standard_email\" UNIQUE(\"name\",\"email\")"
    ...
    describe "runtime behavior" $ do
        ...
        it "standard unique prevents duplicate non-NULLs" $ do
            (runDb $ do
                ...
                _ <- insert $ StandardUnique "user1" (Just "test@example.com")
                return ()) `Hspec.shouldThrow` Hspec.anyException

        it "standard unique getBy returns Nothing for NULL values (backwards compatibility)" $ do
            runDb $ do
                cleanDB
                    ...
                    if supported
                        then do
                            ...
                            liftIO $ case result of
                                ...
                                Nothing -> Hspec.expectationFailure "getBy should find NULL values when !nullsNotDistinct is set"
                        else
                            liftIO $ Hspec.pendingWith "Requires PostgreSQL 15 or higher"
        ...
        it "migration generates correct constraints" $ do
            runDb $ do
                ...
                constraints :: [(Single Text, Single Text)] <- rawSql
                ...
                liftIO $ case constraints of
                    ...
                    results -> do
                        ...
                        let hasNND = any (\(Single _, Single def) -> 
                                "NULLS NOT DISTINCT" `T.isInfixOf` def) results
